...
const Pagination: React.FC<PaginationProps> = ({
  ...
  const displayItemType = itemType || t("pagination.items");

  return (
    <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
      <div className="text-sm text-[var(--muted-foreground)]">
        {t("pagination.showing")} {(pagination.currentPage - 1) * pageSize + 1}{" "}
        {t("pagination.to")}{" "}
        {Math.min(pagination.currentPage * pageSize, pagination.totalCount)}{" "}
        {t("pagination.of")} {pagination.totalCount} {displayItemType}
      </div>
